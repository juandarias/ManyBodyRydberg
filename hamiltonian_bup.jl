"""
# TODO
# -----------------------------------------------------------
# -add argument to function for savefolder location and update functions accordingly: Done
# -add mean field potential hamiltonian
# -import HvdW everywhere in main!
# -----------------------------------------------------------
# ISSUES
# -----------------------------------------------------------
# HvdW cannot be executed in external program. See https://stackoverflow.com/questions/53344955/julia-parallel-function-inside-module
# https://discourse.julialang.org/t/distributed-parallelism-within-packages-applications/14320/5
"""
module hamiltonian


using JLD2, FileIO
using Distributed
using DelimitedFiles
using SparseArrays
using LinearAlgebra


### Laser detuning
function Hatom(Δ, proj_ops, save_location)
    n_atoms = length(proj_ops["Excitation_Operators"])
    dim_b = 2^n_atoms
    H_atom = spzeros(dim_b,dim_b)
    for i in 1:n_atoms
        h_ai = 0.5 * Δ * proj_ops["Projectors"][i].data
        H_atom += h_ai #TODO: is global needed
    end
    save(save_location*"hatom.jld2", "H_atom", H_atom)
end
### Laser detuning

### Population transfer
function Hrabi(Ω, proj_ops, save_location)
    n_atoms = length(proj_ops["Excitation_Operators"])
    dim_b = 2^n_atoms
    H_rabi = spzeros(dim_b,dim_b)
    for i in 1:n_atoms
        h_r = Ω * (proj_ops["Excitation_Operators"][i].data + proj_ops["Decay_Operators"][i].data)
        H_rabi += h_r
    end
    save(save_location*"hrabi.jld2", "H_rabi", H_rabi)
end
### Population transfer


### Ion-Rydberg
function Hion(C4, pos_ops, proj_ops, save_location)
    n_atoms = length(proj_ops["Excitation_Operators"])
    dim_b = 2^n_atoms
    H_ion = spzeros(dim_b,dim_b)
    for i in 1:n_atoms
        #di = pos_ops["Position_x"][i]^2 + pos_ops["Position_y"][i]^2 + pos_ops["Position_z"][i]^2
        di_sq = (pos_ops["Position_x"][i].data).^2 + (pos_ops["Position_y"][i].data).^2 + (pos_ops["Position_z"][i].data).^2
        d4 = abs.(diag(di_sq)).^-2
        d4_matrix = sparse(diagm(0 => d4))
        #d4_operator = SparseOperator(b_mb, b_mb, d4_matrix)
        h_i = C4 * proj_ops["Projectors"][i].data * d4_matrix
        H_ion += h_i
    end
    save(save_location*"hion.jld2", "H_ion", H_ion)
end
### Ion-Rydberg

function h_vdW_ijt(i,j, C6, pos_ops, proj_ops, save_location)
    return i + j
end

### vdW Interaction
function h_vdW_ij(i,j,C6, pos_ops, proj_ops, save_location)
    dx_sq= (pos_ops["Position_x"][i].data - pos_ops["Position_x"][j].data).^2
    dy_sq= (pos_ops["Position_y"][i].data - pos_ops["Position_y"][j].data).^2
    dz_sq= (pos_ops["Position_z"][i].data - pos_ops["Position_z"][j].data).^2
    dij_sq = dx_sq + dy_sq + dz_sq
    d6 = abs.(diag(dij_sq)).^-3
    #d6[d6.==Inf] .=1 #d6 must be a vector
    d6_matrix = spdiagm(0 => d6)
    h_vdw_ij = C6 * sparse(proj_ops["Projectors"][j].data*proj_ops["Projectors"][i].data*d6_matrix) #Conversion to sparse matrix to save bytes
    return h_vdw_ij
end

function HvdW(C6, pos_ops, proj_ops, save_location)
    iter_vdW = []
    n_atoms = length(proj_ops["Excitation_Operators"])
    for i in 1:n_atoms, j in i+1:n_atoms
        push!(iter_vdW, (i,j))
    end

    H_vdW = []
    H_vdW = @distributed (+) for (i,j) in iter_vdW
        h_vdW_ij(i,j, C6, pos_ops, proj_ops, save_location)
    end
    save(save_location*"hvdW.jld2", "H_vdW", H_vdW)
end

### vdW Interaction


### vdW Interaction

function HvdWfull(C6, pos_ops, proj_ops, save_location)
    n_atoms = length(proj_ops["Excitation_Operators"])
    #Matrix elements
    f_vdw_ij = function h_vdW_ij(C6, i,j)
        dij_sq = ((pos_ops["Position_x"][i] - pos_ops["Position_x"][j])^2 + (pos_ops["Position_y"][i] - pos_ops["Position_y"][j])^2)
        d6 = abs.(diag(dij_sq.data)).^-3
        #d6[d6.==Inf] .=1 #d6 must be a vector
        d6_matrix = spdiagm(0 => d6)
        h_vdw_ij = C6 * sparse(proj_ops["Projectors"][j].data*proj_ops["Projectors"][i].data*d6_matrix) #Conversion to sparse matrix to save bytes
        return h_vdw_ij
    end
    
    #Construction operator
    H_vdW, iter_vdW = [], []
    for i in 1:n_atoms, j in i+1:n_atoms
        push!(iter_vdW, (i,j))
    end
    
    pmap(f_vdw_ij, iter_vdW)
    # H_vdW = @distributed (+) for (i,j) in iter_vdW
    #     h_vdW_ij(C6, i,j)
    # end
    save(save_location*"hvdW.jld2", "H_vdW", H_vdW)
end

### vdW Interaction


function HvdWfull2(C6, save_location)
    pos_ops = jldopen(save_location*"pos_ops.jld2", "r", mmaparrays=true)
    proj_ops = jldopen(save_location*"proj_ops.jld2", "r", mmaparrays=true)
    n_atoms = length(proj_ops["Excitation_Operators"])
    #Matrix elements
    f_vdw_ij = function h_vdW_ijtt(iter)
        i = iter[1]
        j = iter[2]
        dij_sq = ((pos_ops["Position_x"][i] - pos_ops["Position_x"][j])^2 + (pos_ops["Position_y"][i] - pos_ops["Position_y"][j])^2)
        d6 = abs.(diag(dij_sq.data)).^-3
        #d6[d6.==Inf] .=1 #d6 must be a vector
        d6_matrix = spdiagm(0 => d6)
        h_vdw_ij = sparse(proj_ops["Projectors"][j].data*proj_ops["Projectors"][i].data*d6_matrix) #Conversion to sparse matrix to save bytes
        return h_vdw_ij
    end
    
    #Construction operator
    H_vdW, iter_vdW = [], []
    for i in 1:n_atoms, j in i+1:n_atoms
        push!(iter_vdW, (i,j))
    end
    result = pmap(f_vdw_ij, iter_vdW)

    # H_vdW = @distributed (+) for (i,j) in iter_vdW
    #     h_vdW_ij(C6, i,j)
    # end
    save(save_location*"hvdW.jld2", "H_vdW", H_vdW)
    return result
end




function HvdWfull3(C6, save_location)
    pos_ops = jldopen(save_location*"pos_ops.jld2", "r", mmaparrays=true)
    proj_ops = jldopen(save_location*"proj_ops.jld2", "r", mmaparrays=true)
    n_atoms = length(proj_ops["Excitation_Operators"])
    #Matrix elements
    function h_vdW_ijt(C6, i,j)
        dij_sq = ((pos_ops["Position_x"][i] - pos_ops["Position_x"][j])^2 + (pos_ops["Position_y"][i] - pos_ops["Position_y"][j])^2)
        d6 = abs.(diag(dij_sq.data)).^-3
        #d6[d6.==Inf] .=1 #d6 must be a vector
        d6_matrix = spdiagm(0 => d6)
        h_vdw_ij = C6 * sparse(proj_ops["Projectors"][j].data*proj_ops["Projectors"][i].data*d6_matrix) #Conversion to sparse matrix to save bytes
        return h_vdw_ij
    end
    
    #Construction operator
    H_vdW, iter_vdW = [], []
    for i in 1:n_atoms, j in i+1:n_atoms
        push!(iter_vdW, (i,j))
    end
    #pmap(f_vdw_ij, iter_vdW)

    H_vdW = @distributed (+) for (i,j) in iter_vdW
        h_vdW_ijt(C6, i,j)
    end
    save(save_location*"hvdW.jld2", "H_vdW", H_vdW)
end


### Full Hamiltonian
function Htotal(save_location)
    
    #Open JLD files
    fatom = jldopen(save_location*"hatom.jld2", mmaparrays=true)
    fion = jldopen(save_location*"hion.jld2", mmaparrays=true)
    frabi = jldopen(save_location*"hrabi.jld2", mmaparrays=true)
    fvdW = jldopen(save_location*"hvdW.jld2", mmaparrays=true)
    
    
    #Generate empty sparse array for Hamiltonian, save to disk and empty allocation
    dim_b = size(fatom["H_atom"])[1]
    H_total_0 = spzeros(dim_b,dim_b)
    save(save_location*"htotal_0.jld2", "H_total", H_total_0)
    H_total_0 = nothing


    #Create Hamiltomnian
    ftotal = jldopen(save_location*"htotal_0.jld2", mmaparrays=true)
    ftotal["H_total"] .+= fatom["H_atom"]
    ftotal["H_total"] .+= fion["H_ion"]
    ftotal["H_total"] .+= frabi["H_rabi"]
    ftotal["H_total"] .+= fvdW["H_vdW"]
    #Save Hamiltonian
    save(save_location*"htotal.jld2", "H_total", ftotal["H_total"])
    close(ftotal), close(fatom), close(frabi), close(fion), close(fvdW)
end
### Full Hamiltonian


### Hamiltonian no vdW
function HtotalnovdW(save_location)
    
    #Open JLD files
    fatom = jldopen(save_location*"hatom.jld2", mmaparrays=true)
    fion = jldopen(save_location*"hion.jld2", mmaparrays=true)
    frabi = jldopen(save_location*"hrabi.jld2", mmaparrays=true)
    #fvdW = jldopen(save_location*"hvdW.jld2", mmaparrays=true)
    
    
    #Generate empty sparse array for Hamiltonian, save to disk and empty allocation
    dim_b = size(fatom["H_atom"])[1]
    H_total_0 = spzeros(dim_b,dim_b)
    save(save_location*"htotal_0.jld2", "H_total", H_total_0)
    H_total_0 = nothing


    #Create Hamiltomnian
    ftotal = jldopen(save_location*"htotal_0.jld2", mmaparrays=true)
    ftotal["H_total"] .+= fatom["H_atom"]
    ftotal["H_total"] .+= fion["H_ion"]
    ftotal["H_total"] .+= frabi["H_rabi"]
    #ftotal["H_total"] .+= fvdW["H_vdW"]
    #Save Hamiltonian
    save(save_location*"htotal.jld2", "H_total", ftotal["H_total"])
    close(ftotal), close(fatom), close(frabi), close(fion) #, close(fvdW)
end
### Hamiltonian no vdW


end