...
### Laser detuning
function hatom_i(i::Int8, Δ::Float64, save_location::AbstractString)
...
function Hatom(Δ::Float64, proj_ops, save_location::AbstractString)
    ...
    dim_b = 2^n_atoms
    H_atom = spzeros(dim_b,dim_b)
    H_atom = @distributed (+) for i in 1:n_atoms
        hatom_i(i, Δ, save_location)
    end
    save(save_location*"hatom.jld2", "H_atom", H_atom)
end
### Laser detuning
...
function hrabi_i(i::Int8, Ω::Float64, save_location::AbstractString)
    proj_ops = jldopen(save_location*"proj_ops.jld2", "r", mmaparrays=true)
...
function Hrabi(Ω::Float64, proj_ops, save_location::AbstractString)
    ...
    H_rabi = @distributed (+) for i in 1:n_atoms
        hrabi_i(i, Ω, save_location)
    end
...
### Ion-Rydberg
function hion_i(i::Int8, C4::Float64, save_location::AbstractString)
    proj_ops = jldopen(save_location*"proj_ops.jld2", "r", mmaparrays=true)
...
function Hion(C4::Float64, pos_ops, proj_ops, save_location::AbstractString)
    ...
    H_ion = @distributed (+) for i in 1:n_atoms
        hion_i(i, C4, save_location)
    end
...
### Paul trap + ion
function hionpt_i(i::Int8, topology, save_location::AbstractString)
...
function HPTIon(topology, pos_ops, proj_ops, time_simulation::Float64, save_location::AbstractString)
    ...
    H_PTIon @distributed (+) for i in 1:n_atoms
        hionpt_i(i, topology,  save_location)
    end
